...
<input type="button" class="clear" value="Clear All Filters" onClick="window.location.href=window.location.href">
<div id="filter"><br>
...
// Select statement
$query = "SELECT * FROM $dbname.glasses WHERE `glasses` LIKE '%$keyword%'";
...
if (!empty($row['static'])){
	echo "<tr>";
	echo '<td align="center"`>'.'<input type="button" style ="background-color:#1e1547; color:white;"name="static.php?id='.$row['id'].'&shape='.$row['shape'].'&gender='.$row['gender'].'&static='.$row['static'].'" class="my-button" value="Static Try On"/>'."</td>";
	echo "</tr>";
}	
	echo "</tr>";
	echo "</tbody>";
...
if ($anymatches == 0) 
{ 
   echo "<br><br>Nothing was found that matched your search - Please Try Again By Clicking Below<br><br>"; 
}
